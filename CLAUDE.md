...
4. Copy existing state into DB: `npm run build:agent && node --env-file=.env.local packages/agent/build/migrate-state-to-db.js [--force]`.
5. Flip the flag on the agent host (Render): `TEND_STATE_BACKEND=db`.
6. Optional — flip the same flag on the frontend (Vercel) so API routes read directly from Postgres instead of the `/state` proxy.
...
Schema lives in `packages/shared/src/db/schema.ts`; migrations generated by `npm run db:generate -w packages/shared` land in `packages/shared/drizzle/`.
